...
If Git pauses due to a conflict, it means it hit a conflict while replaying one of your commits. **This cycle can repeat once per commit** — if you have 3 commits and each touches the same file, you may resolve conflicts 3 separate times.

For each pause, repeat this cycle:

1. Open your IDE and manually resolve the conflict markers in the affected file(s):
```
<<<<<<< HEAD          ← origin/main's version
...
=======
...
>>>>>>> abc1234...    ← your commit's version
```
...
4. **Git will open Vim** (or your default editor) showing the commit message for the commit it just replayed. You usually don't need to change anything — just save and exit:
   - **Vim**: Press `Esc` → type `:wq` → `Enter`
   - **Nano**: `Ctrl+X` → `Y` → `Enter`

5. Git moves to the next commit. If that commit also has a conflict, it pauses again — **repeat from step 1**.

When all commits are replayed with no more conflicts, the rebase is complete.
