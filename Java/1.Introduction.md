...
1) In Java, arrays are part of the language syntax, not full objects from a class like Array —
...
2) The deeper reason (under the hood)
    ...
    Java’s design philosophy (since the early days):
    ```
    ...
    ```
        ...
        That’s why:
            ...
            - Collections → static methods for List, Set, Map

            - Math → static math operations

            - Objects → static null-safe helpers
    ...
    - Arrays in Java are not like JavaScript or Python lists
        ...
        Java doesn’t — it’s statically typed and JVM-backed, so array behavior is baked in at the VM level for speed.
...
class Main {
    public static void main(String[] args) {
        ...
        System.out.println(inrAmount); // ₹1,234,567.89
