<div id="top"></div>

# Java Utilities & Collections

<summary>Table of contents</summary>

- [Common Utility Methods](#common-utility-methods)
- [Java Arrays](#java-arrays)
- [Collections](#collections)
  - [List](#list)
    - [ArrayList](#arraylist)
    - [LinkedList](#linkedlist)
    - [Vector](#vector)
  - [Set](#set)
    - [HashSet](#hashset)
    - [TreeSet](#treeset)
  - [Map](#map)
    - [HashMap](#hashmap)
  - [Queue](#queue)
    - [PriorityQueue](#priorityqueue)
    - [Deque](#deque)

</details>

## Common Utility Methods

### String Class
  - it is a reference type
  - strings are immutable (we cant change the string)
  - any methods that changes a string returns a new string

|Method|Purpose|Example|
|------|-------|-------|
|`length()`|Returns string length	|`"Hello".length()` → `5`|
|`isEmpty()`|Checks if string is empty|`"hello".isEmpty()` → `false`|
|`equals(str)`|Compares string values|`"Hello".equals("Hello")`|
|`indexOf(str)`|Returns starting index of substring|`"Hello".indexOf("He")` → `0`|
|`indexOf(val)`|Returns index of first occurrence|`"Hello".indexOf("o")` → `4`|
|`substring(start)`|Extracts from index → end|`"Hello".substring(2)` → `"llo"`|
|`substring(start,end)`|Extracts part of string	|`"Hello".substring(1,4)` → `"ell"`|

## StringBuilder Class

- It is a **mutable sequence of characters**
- Efficient for **frequent modifications**

- **Declaration**

```java
StringBuilder sb = new StringBuilder("Hello");
```
|Method|Purpose|Example|
|------|-------|-------|
|`append(val)`| Adds value at end	|	`sb.append(" World");` |
|`insert(index, val)`| Inserts at given index	|	`sb.insert(1, "X");` |
|`delete(start, end)`| Deletes substring	|	`sb.delete(1,3);`|
|`deleteCharAt(index)`|	Deletes character at index |	`sb.deleteCharAt(2);`|
|`replace(start, end, str)`|	Replaces substring |	`sb.replace(1,3,"AB");`|
|`reverse()`|	Reverses string |	`sb.reverse();`|
|`toString()`|	Converts to String |	`sb.toString();`|
|`charAt(index)`|	Returns character at index |	`sb.charAt(0);`|
|`setCharAt(index, ch)`|	Updates character |	`sb.setCharAt(0,'H');`|
|`length()`|	Returns current length |	`sb.length();`|
|`setLength()`|	sets the length |	`sb.setLength();`|
|`indexOf(str)`| Returns index of first occurrence of substring |	`sb.indexOf("lo");` |
|`lastIndexOf(str)`| Returns index of last occurrence of substring |	`sb.lastIndexOf("l");` |
|`substring(start)`| Extracts from index to end (returns String) |	`sb.substring(2);` |
|`substring(start, end)`| Extracts part of the sequence (returns String) |	`sb.substring(1,4);` |
|`capacity()`| Returns current storage capacity |	`sb.capacity();` |
|`ensureCapacity(min)`| Ensures capacity is at least the given value |	`sb.ensureCapacity(50);` |
|`trimToSize()`| Trims capacity down to current length |	`sb.trimToSize();` |

### Integer Class

|Method|Purpose|Example|
|------|-------|-------|
|`Integer.toString(num)`|	Converts integer → string|	`Integer.toString(1234)` → `"1234"`|
|`Integer.toBinaryString(num)`|	Converts integer → binary string|	`Integer.toBinaryString(5)` → `"101"`|
|`Integer.parseInt(str)`|	Converts string → integer|	`Integer.parseInt("11")` → `11`|
|`Integer.parseInt(str, radix)`|	Converts number in base → decimal|	`Integer.parseInt("1101",2)` → `13`|

### Math Class

|Method|Purpose|Example|
|------|-------|-------|
|`Math.max(a,b)`|	Returns the larger of two numbers|	`Math.max(1,2)` → `2`|
|`Math.floor(num)`|	Returns largest integer ≤ number	|`Math.floor(4.9)` → `4.0`|
|`Math.ceil(num)`|	Returns smallest integer ≥ number|	`Math.ceil(4.1)` → `5.0`|
|`Math.abs(num)`	|Absolute value	|`Math.abs(-5)` → `5`|
|`Math.pow(a,b)`|	a raised to power b|	`Math.pow(2,3)` → `8.0`|
|`Math.sqrt(num)`|	Square root	|`Math.sqrt(16)` → `4.0`|
|`Math.round(num)`|	Rounds to nearest integer|`	Math.round(4.6)` → `5`|

### Character Class

| Method | Description | Example |
|---|---|---|
| `Character.isLetter(c)` | Checks if character is a letter | `Character.isLetter('A')` → `true` |
| `Character.isDigit(c)` | Checks if character is a digit | `Character.isDigit('5')` → `true` |
| `Character.toLowerCase(c)` | Converts character to lowercase | `Character.toLowerCase('A')` → `'a'` |
| `Character.toUpperCase(c)` | Converts character to uppercase | `Character.toUpperCase('z')` → `'Z'` |
| `Character.getType(c)` | Returns integer representing character type | `Character.getType('A')` → `1` |

### String & Array Conversion

|Method|Purpose|Example|
|---|---|---|
|`toCharArray()`|convert string → char array|`char[] arr = s.toCharArray(); `|
|`new String(char[])`|char array → string |`String s = new String(arr); `|
|`String.valueOf()`|primitive → string |`String s = String.valueOf(123); `|


### Escape Sequence

```java
String s = "I'm \n \"Escape Sequence\" \t C:\\Windows";
System.out.println(s);
```

### Constants

- to declare a constant we user final keyword

```java
final float pi = 3.14F; // if we don't declare F it is seen as decimal
System.out.println("Pi is " + pi); // Pi is 3.14
```

### Casting

- Implicit Casting (no data loss): byte > Short > int > long > float > double
```java
short x = 1;
int y = x + 1; // implicit casting
System.out.println(y);
```

- Explicit Casting (data loss): double > float > long > int > char > short > byte
```java
double x = 1.1;
int y = (int)x + 2; // Explicit casting
System.out.println(y); // 3
```

## Order of operations

() -> */ -> +-

## Java Arrays

- **method overloading:** multiple methods with the same name but different parameter (Ex: Arrays.toString())
- **method overridding:** the method of the subclass class overrides the method of the superclass

```java
Single Dimension Array:

    int[] numbers = new int[5]; // declaration - uninitialized values: 0
    int[] numbers = new int[]{3,4}; // declaration and initialization (Shouldn't declare the size to initialize)
    int[] numbers = {2,3,4,1,5};
    System.out.println(Arrays.toString(numbers)); // [2, 3, 4, 1, 5]
    Arrays.sort(numbers);
    System.out.println(Arrays.toString(numbers)); // [1, 2, 3, 4, 5]

Multi Dimension Array:

    int[][] numbers = new int[][]{{1,2,3},{0,0,0}};
    System.out.println(Arrays.deepToString(numbers)); // [[1, 2, 3], [0, 0, 0]]
    int[][] multiArray = {{1,2},{0,0,0}};
    System.out.println(Arrays.deepToString(multiArray)); // [[1, 2], [0, 0, 0]]
    int[][] multiArray2 = new int[2][3];
    System.out.println(Arrays.deepToString(multiArray2)); // [[0, 0, 0], [0, 0, 0]]


```

- **Static return**

```java
return new int[] {1,2,3};
```

- **Methods**

| Method | Description | Example |
|---|---|---|
| `length` | Returns size of array | `arr.length` |
| `sort(arr)` | Sorts array in ascending order | `Arrays.sort(arr)` |
| `equals(arr1,arr2)` | Checks two arrays are equal | `Arrays.equals(arr1,arr2)` |
| `toString(arr)` | Converts array to string | `Arrays.toString(arr)` |
| `deepToString(arr)` | Converts multi-dimension array to string | `Arrays.deepToString(numbers)` |

<p align="right">(<a href="#top">back to top</a>)</p>

<hr/>

## Collections

## List

### ArrayList

- dynamic array, not synchronized not suitable when multiple threads are accessing it simultaneously

- **Declaration**

```java
ArrayList<int[]> ans = new ArrayList<int[]>(); we should pass non primitive data type
ArrayList<Integer> al = new ArrayList<>(10); we added 10 as capacity
ArrayList <Integer> al2 = new ArrayList<>(Arrays.asList(1,2,3));
```

- **Methods**

| Method | Description | Example |
|---|---|---|
| `add(val)` | Adds element to end of list | `al.add(10);` |
| `add(index, val)` | Inserts element at specified index | `al.add(1, 20);` |
| `get(index)` | Returns element at index | `al.get(0);` |
| `set(index, val)` | Updates element at index | `al.set(0, 99);` |
| `remove(index)` | Removes element at index | `al.remove(2);` |
| `contains(val)` | Checks if list contains element | `al.contains(50);` |
| `size()` | Returns number of elements | `al.size();` |
| `clear()` | Removes all elements | `al.clear();` |
| `toArray()` | Converts list to array | `al.toArray();` |
| `isEmpty()` | Checks if list is empty | `al.isEmpty();` |
| `indexOf(val)` | Returns index of first occurrence | `al.indexOf(99);` |
| `lastIndexOf(val)` | Returns index of last occurrence | `al.lastIndexOf(99);` |
| `addAll(Arrays.asList(arr))` | Adds array elements to ArrayList | `list.addAll(Arrays.asList(arr));` |
| `addAll(list)` | Adds elements from another list | `list.addAll(list2);` |
| `al.stream().mapToInt(i -> i).toArray()` | Converts `ArrayList<Integer>` to `int[]` | `int[] arr = al.stream().mapToInt(i -> i).toArray();` |
| `Collections.reverse(list)` | Reverses list order | `Collections.reverse(list);` |

<p align="right">(<a href="#top">back to top</a>)</p>

<hr/>

### Vector

- synchronized, thread-safe, but slower than ArrayList

- **Declaration**

```java
Vector<Integer> v = new Vector<Integer>();
```

- **Methods**
<h3>📘 Vector – Useful Methods</h3>

| Method | Description | Example |
|---|---|---|
| `add(val)` | Adds element to end | `v.add(10);` |
| `add(index, val)` | Inserts element at index | `v.add(1, 20);` |
| `get(index)` | Gets element at index | `v.get(0);` |
| `set(index, val)` | Updates element at index | `v.set(0, 50);` |
| `remove(index)` | Removes element at index | `v.remove(2);` |
| `size()` | Returns number of elements | `v.size();` |
| `contains(val)` | Checks if value exists | `v.contains(100);` |
| `clear()` | Removes all elements | `v.clear();` |
| `addAll(Collection)` | Adds all elements from another collection | `v.addAll(list);` |
| `elementAt(index)` | Gets element at index (legacy method) | `v.elementAt(0);` |
| `firstElement()` | Returns first element | `v.firstElement();` |
| `lastElement()` | Returns last element | `v.lastElement();` |

<p align="right">(<a href="#top">back to top</a>)</p>

<hr/>

## Set

### HashSet

- A HashSet is a data structure that stores **unique elements only**
- Does **not maintain order** (no insertion order, no sorting)
- Internally uses a **HashMap**
- Not synchronized, not thread-safe

- **Declaration**

```java
HashSet<Integer> set = new HashSet<>();
HashSet<String> set2 = new HashSet<>(Arrays.asList("a", "b", "c"));
```

- **Methods**

| Method | Description | Example |
|---|---|---|
| `add(val)` | Adds element (returns false if already exists) | `set.add(10);` |
| `remove(val)` | Removes element if present | `set.remove(10);` |
| `contains(val)` | Checks if element exists | `set.contains(5);` |
| `size()` | Returns number of elements | `set.size();` |
| `clear()` | Removes all elements | `set.clear();` |
| `isEmpty()` | Checks if set is empty | `set.isEmpty();` |
| `iterator()` | Returns iterator (no guaranteed order) | `set.iterator();` |
| `for-each` | Iterate through elements | `for(int x : set) {}` |
| `addAll(collection)` | Adds all elements from collection | `set.addAll(list);` |
| `removeAll(collection)` | Removes all elements from collection | `set.removeAll(list);` |
| `retainAll(collection)` | Keeps only common elements | `set.retainAll(list);` |
| `toArray()` | Converts set to array | `set.toArray();` |

<p align="right">(<a href="#top">back to top</a>)</p>

<hr/>

### TreeSet

- A TreeSet is a sorted set that stores unique elements in ascending order (default)
- Internally uses a **Red-Black Tree**
- Not synchronized, not thread-safe

- **Declaration**

```java
TreeSet<Integer> ts = new TreeSet<>(); // default ascending order

TreeSet<Integer> desc = new TreeSet<>((a, b) -> b - a); // descending order

TreeSet<Integer> ts2 = new TreeSet<>(Arrays.asList(3, 1, 2));
```

- **Methods**

| Method | Description | Example |
|---|---|---|
| `add(val)` | Adds element (no duplicates allowed) | `ts.add(10);` |
| `remove(val)` | Removes element | `ts.remove(10);` |
| `contains(val)` | Checks if element exists | `ts.contains(5);` |
| `first()` | Returns smallest element | `ts.first();` |
| `last()` | Returns largest element | `ts.last();` |
| `higher(val)` | Returns next greater element | `ts.higher(10);` |
| `lower(val)` | Returns next smaller element | `ts.lower(10);` |
| `ceiling(val)` | Returns smallest element ≥ val | `ts.ceiling(10);` |
| `floor(val)` | Returns largest element ≤ val | `ts.floor(10);` |
| `pollFirst()` | Removes & returns smallest element | `ts.pollFirst();` |
| `pollLast()` | Removes & returns largest element | `ts.pollLast();` |
| `size()` | Returns number of elements | `ts.size();` |
| `clear()` | Removes all elements | `ts.clear();` |
| `isEmpty()` | Checks if empty | `ts.isEmpty();` |
| `iterator()` | Returns iterator (sorted order) | `ts.iterator();` |

- **Important Points**

- Elements are always **sorted**
- No duplicates allowed
- All operations → **O(log n)**
- Used when you need **sorted + unique elements**

<hr/>

## Map

### HashMap

- A HashMap is a data structure that stores key-value pairs.

- **Declaration**

```java
HashMap<Integer,Integer> store = new HashMap<>();
```

- **Methods**

| Method | Description | Example |
|---|---|---|
| `put(key, value)` | Inserts or updates a key-value pair | `map.put(1, 100);` |
| `get(key)` | Returns the value for the key or `null` if not found | `map.get(1);` |
| `getOrDefault(key, default value)` | Returns the value for the key or default val if not found | `map.getOrDefault(1, 0);` |
| `computeIfAbsent(key, func)` | Adds value for key if absent and returns it | `map.computeIfAbsent("A", k -> new ArrayList<>());` |
| `containsKey(key)` | Checks if the map contains the key | `map.containsKey(1);` |
| `remove(key)` | Removes key-value pair for the given key | `map.remove(1);` |
| `size()` | Returns number of key-value pairs | `map.size();` |
| `clear()` | Removes all mappings | `map.clear();` |
| `isEmpty()` | Checks if the map is empty | `map.isEmpty();` |
| `keySet()` | Returns a `Set` view of keys | `map.keySet();` |
| `values()` | Returns a collection of values | `map.values();` |
| `entrySet()` | Returns a `Set` of key-value pairs | `map.entrySet();` |
| `for-each (entrySet)` | Iterate through map entries | `for(Map.Entry<Integer,Integer> e : map.entrySet()) { e.getKey(); e.getValue(); }` |
| `sort by value` | Sort map entries by value | `list.sort((a,b) -> b.getValue() - a.getValue());` |

<p align="right">(<a href="#top">back to top</a>)</p>

<hr/>

## Queue

### PriorityQueue

- A PriorityQueue is a heap-based data structure where elements are ordered based on priority (default: min-heap)
- Not synchronized, not thread-safe

---

- **Comparator — How It Works**

- Comparator's `compare(a, b)` return value decides order:
  - **negative** → `a` comes before `b` (a has higher priority)
  - **positive** → `b` comes before `a`
  - **zero** → tie, order unspecified
- PriorityQueue always polls the element the comparator ranks *first*

---

- **Comparator Long Form vs Lambda (same thing)**

```java
// Long form (pre-Java 8, you'll see this in old tutorials)
new PriorityQueue<>(new Comparator<Integer>() {
    @Override
    public int compare(int a, int b) {
        return a - b;
    }
});

// Lambda (Java 8+) — always use this in DSA/interviews
new PriorityQueue<>((a, b) -> a - b);
```

- They are **identical at runtime** — lambda is just the short form of the anonymous class
- Always use lambda in DSA/interviews — shorter, no overhead
- `Integer.compare(a, b)` is the overflow-safe version of `a - b`, not a different concept

---

- **When to Use What**

| Situation | Use |
|---|---|
| Simple integers, LeetCode-style | `a - b` (fine, concise) |
| Values near `Integer.MIN_VALUE` / `MAX_VALUE` | `Integer.compare(a, b)` (overflow-safe) |
| `long`, `double`, `float` | `Long.compare()`, `Double.compare()` |
| Strings, chars | `a.compareTo(b)` |
| Custom objects | `Integer.compare(a.field, b.field)` |
| Reading old code | Long form — just know it's the same thing |

---

- **Declaration**

```java
// --- Integers ---
PriorityQueue<Integer> minHeap = new PriorityQueue<>();                                    // min-heap (natural ordering)
PriorityQueue<Integer> minHeapExplicit = new PriorityQueue<>((a, b) -> a - b);             // min-heap, explicit
PriorityQueue<Integer> minHeapSafe = new PriorityQueue<>((a,b)->Integer.compare(a,b));     // min-heap, overflow-safe
PriorityQueue<Integer> maxHeap = new PriorityQueue<>((a, b) -> b - a);                     // max-heap, subtraction
PriorityQueue<Integer> maxHeapSafe = new PriorityQueue<>((a, b) -> Integer.compare(b,a));  // max-heap, safe    
PriorityQueue<Integer> maxHeapClean = new PriorityQueue<>(Comparator.reverseOrder());      // max-heap, cleanest

// --- Arrays ---
PriorityQueue<int[]> pqArrMin = new PriorityQueue<>((a, b) -> a[0] - b[0]);                     // min on arr[0]
PriorityQueue<int[]> pqArrMinSafe = new PriorityQueue<>((a, b) -> Integer.compare(a[0], b[0])); // min on arr[0], safe
PriorityQueue<int[]> pqArrMax = new PriorityQueue<>((a, b) -> Integer.compare(b[0], a[0]));     // max on arr[0], safe

// --- Multi-key (arrays) ---
PriorityQueue<int[]> pqMultiKey = new PriorityQueue<>((a, b) -> {
    if (a[0] != b[0]) return Integer.compare(a[0], b[0]); // primary: arr[0] ascending
    return Integer.compare(b[1], a[1]);                   // tie-break: arr[1] descending
});

// --- From collection ---
PriorityQueue<Integer> pqFromList = new PriorityQueue<>(Arrays.asList(1, 3, 2)); // heapifies on insert
```

---

- **Methods**

| Method | Description | Example |
|---|---|---|
| `offer(val)` | Inserts element (preferred) | `pq.offer(10);` |
| `add(val)` | Inserts element (throws if fails) | `pq.add(20);` |
| `peek()` | Returns top without removing | `pq.peek();` |
| `poll()` | Removes and returns top | `pq.poll();` |
| `remove()` | Removes top (throws if empty) | `pq.remove();` |
| `remove(val)` | Removes specific element | `pq.remove(10);` |
| `contains(val)` | Checks if element exists | `pq.contains(5);` |
| `size()` | Returns number of elements | `pq.size();` |
| `clear()` | Removes all elements | `pq.clear();` |
| `isEmpty()` | Checks if empty | `pq.isEmpty();` |
| `iterator()` | Returns iterator (not sorted order) | `pq.iterator();` |
| `toArray()` | Converts to array | `pq.toArray();` |

---

-  **Quick Mental Model**

- `a - b` → min-heap shortcut, fine for LeetCode
- `b - a` → max-heap shortcut, fine for LeetCode
- `Integer.compare(a, b)` → same as `a - b` but overflow-safe
- `Integer.compare(b, a)` → same as `b - a` but overflow-safe
- Long form (`new Comparator<>() { ... }`) → old tutorials only, never write fresh

<p align="right">(<a href="#top">back to top</a>)</p>

<hr/>

### Deque

- A Deque (Double Ended Queue) allows insertion and deletion from both ends
- Can be used as **queue (FIFO)** or **stack (LIFO)**
- Common implementations:
  - `ArrayDeque` (fast, preferred)
  - `LinkedList`
- Not synchronized, not thread-safe

- **Declaration**

```java
Deque<Integer> dq = new ArrayDeque<>();

Deque<Integer> dq2 = new LinkedList<>();

Deque<Integer> dq3 = new ArrayDeque<>(Arrays.asList(1, 2, 3));
```
- **Methods**

| Method | Description | Example |
|---|---|---|
| `addFirst(val)` | Adds element at front | `dq.addFirst(10);` |
| `addLast(val)` | Adds element at rear | `dq.addLast(20);` |
| `offerFirst(val)` | Adds at front (returns false if fails) | `dq.offerFirst(5);` |
| `offerLast(val)` | Adds at rear (returns false if fails) | `dq.offerLast(15);` |
| `removeFirst()` | Removes & returns front element | `dq.removeFirst();` |
| `removeLast()` | Removes & returns last element | `dq.removeLast();` |
| `pollFirst()` | Removes & returns front (null if empty) | `dq.pollFirst();` |
| `pollLast()` | Removes & returns last (null if empty) | `dq.pollLast();` |
| `getFirst()` | Returns front element | `dq.getFirst();` |
| `getLast()` | Returns last element | `dq.getLast();` |
| `peekFirst()` | Returns front (null if empty) | `dq.peekFirst();` |
| `peekLast()` | Returns last (null if empty) | `dq.peekLast();` |
| `push(val)` | Pushes element (stack behavior) | `dq.push(10);` |
| `pop()` | Removes top element (stack behavior) | `dq.pop();` |
| `size()` | Returns number of elements | `dq.size();` |
| `clear()` | Removes all elements | `dq.clear();` |
| `isEmpty()` | Checks if empty | `dq.isEmpty();` |
| `iterator()` | Returns iterator (front to rear) | `dq.iterator();` |


<p align="right">(<a href="#top">back to top</a>)</p>

<hr/>